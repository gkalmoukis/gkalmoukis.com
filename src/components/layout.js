...
const Layout = ({
  ...
  React.useEffect(() => {
    fetch("https://api.github.com/users/gkalmoukis/repos")
      .then(res => res.json())
      .then(data => {
        let totalStars = 0;
        let totalForks = 0;
        console.log(data);
        data.forEach(item => {
          totalStars = totalStars + item.stargazers_count;
          totalForks = totalForks + item.forks_count;
        });
        
        setGithubStars(totalStars);
        setGithubForks(totalForks);
      });
                    ...
                    >
                      <Box>
                        ...
                        >
                          ...
                          <NewsletterDrawer open={state["bottom"]} onClose={toggleDrawer("bottom", false)} /> 
                        </Box>
                        <Typography
                          variant="body2"
